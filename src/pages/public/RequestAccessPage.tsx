...
interface AccessForm { first_name: string; last_name: string; email: string; phone: string; relationship: string; verifier: string; message: string }
      ...
      <div className="container request-grid">
        ...
        <div className="form-card">
          <form onSubmit={submit} className="stack-form">
            <div className="field-grid"><label>First name<input required autoComplete="given-name" value={form.first_name} onChange={e => change('first_name', e.target.value)} /></label><label>Last name<input required autoComplete="family-name" value={form.last_name} onChange={e => change('last_name', e.target.value)} /></label></div>
            <label>Email address<input required type="email" autoComplete="email" value={form.email} onChange={e => change('email', e.target.value)} /></label>
            <label>Phone <span className="optional">Optional</span><input type="tel" autoComplete="tel" value={form.phone} onChange={e => change('phone', e.target.value)} /></label>
            <label>Relationship to the Reetz family<textarea required rows={3} value={form.relationship} onChange={e => change('relationship', e.target.value)} placeholder="For example: grandchild of…" /></label>
            <label>Who invited you or can verify you?<input required value={form.verifier} onChange={e => change('verifier', e.target.value)} placeholder="Name of a family member" /></label>
            <label>Anything else we should know? <span className="optional">Optional</span><textarea rows={4} value={form.message} onChange={e => change('message', e.target.value)} /></label>
